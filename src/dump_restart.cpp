...
DumpRestart::DumpRestart(SPPARKS *spk, int narg, char **arg) : Dump(spk, narg, arg)
{
  ...
  if (ioptional < narg && strcmp(style,"image") != 0)
    error->all(FLERR,"Invalid attribute in dump Restart command");
  ...
  if (binary) header_choice = &DumpRestart::header_binary;
  else header_choice = &DumpRestart::header_text;

  if (binary) write_choice = &DumpRestart::write_binary;
  else write_choice = &DumpRestart::write_text;
}
...
void DumpRestart::init_style()
{
  ...
  int flag = 0;
  for (int i = 0; i < size_one; i++)
    if (pack_choice[i] == &DumpRestart::pack_propensity) flag = 1;
  ...
  if (iregion >= 0) {
    ...
    if (iregion == -1)
      error->all(FLERR,"Region ID for dump Restart does not exist");
  }
...
void DumpRestart::write_header(bigint ndump, double time)
{
...
void DumpRestart::write_data(int n, double *buf)
{
...
void DumpRestart::header_binary(bigint ndump, double time)
{
...
void DumpRestart::header_text(bigint ndump, double time)
{
  //LC modify, comment the header and add values as the end
  // this is for restart and read sites
...
void DumpRestart::write_binary(int n, double *buf)
{
  n *= size_one;
...
void DumpRestart::write_text(int n, double *buf)
{
  int i,j;
...
int DumpRestart::parse_fields(int narg, char **arg)
{
  int i;
  for (int iarg = 4; iarg < narg; iarg++) {
    i = iarg-4;
    ...
    if (strcmp(arg[iarg],"id") == 0) {
      pack_choice[i] = &DumpRestart::pack_id;
      vtype[i] = TAGINT;
    } else if (strcmp(arg[iarg],"site") == 0) {
      pack_choice[i] = &DumpRestart::pack_site;
      vtype[i] = INT;
      if (app->iarray == NULL)
	error->all(FLERR,"Dumping a quantity application does not support");
    } else if (strcmp(arg[iarg],"x") == 0) {
      pack_choice[i] = &DumpRestart::pack_x;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"y") == 0) {
      pack_choice[i] = &DumpRestart::pack_y;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"z") == 0) {
      pack_choice[i] = &DumpRestart::pack_z;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"energy") == 0) {
      pack_choice[i] = &DumpRestart::pack_energy;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"propensity") == 0) {
      pack_choice[i] = &DumpRestart::pack_propensity;
      vtype[i] = DOUBLE;

    }else if (strcmp(arg[iarg],"ctNi") == 0) { // ct_site by LC
        pack_choice[i] = &DumpRestart::pack_ct_site_Ni;
        vtype[i] = DOUBLE;
    }else if (strcmp(arg[iarg],"ctVac") == 0) { // ct_site by LC
        pack_choice[i] = &DumpRestart::pack_ct_site_Vac;
        vtype[i] = DOUBLE;
    }else if (strcmp(arg[iarg],"ctCr") == 0) { // ct_site by LC
        pack_choice[i] = &DumpRestart::pack_ct_site_Cr;
        vtype[i] = DOUBLE;
    }else if (strcmp(arg[iarg],"i3_0") == 0) { // i3_site_0 by LC
        pack_choice[i] = &DumpRestart::pack_i3_site_0;
        vtype[i] = DOUBLE;
    }else if (strcmp(arg[iarg],"i3_1") == 0) { // i3_site_1 by LC
        pack_choice[i] = &DumpRestart::pack_i3_site_1;
        vtype[i] = DOUBLE;
    ...
    // double value = dN
      ...
      pack_choice[i] = &DumpRestart::pack_iarray;
      vtype[i] = INT;
	...
	error->all(FLERR,"Invalid keyword in dump command");
      ...
      pack_choice[i] = &DumpRestart::pack_darray;
      vtype[i] = DOUBLE;
      vindex[i] = atoi(&arg[iarg][1]);
...
int DumpRestart::modify_param(int narg, char **arg)
{
...
void DumpRestart::pack_id(int n)
{
  tagint *id = app->id;
...
void DumpRestart::pack_site(int n)
{
  int *site = app->iarray[0];
...
void DumpRestart::pack_energy(int n)
{
  if (latticeflag) {
    for (int i = 0; i < nchoose; i++) {
...
void DumpRestart::pack_propensity(int n)
{
  if (latticeflag) {
    for (int i = 0; i < nchoose; i++) {
...
void DumpRestart::pack_ct_site_Ni(int n)
{
...
void DumpRestart::pack_ct_site_Vac(int n)
{
  for (int i = 0; i < nchoose; i++) {
...
void DumpRestart::pack_ct_site_Cr(int n)
{
  for (int i = 0; i < nchoose; i++) {
...
/* ---------------------------------------------------------------------- */
void DumpRestart::pack_i3_site_0(int n)
{
...
/* ---------------------------------------------------------------------- */
void DumpRestart::pack_i3_site_1(int n)
{
  for (int i = 0; i < nchoose; i++) {
...
void DumpRestart::pack_iarray(int n)
{
...
void DumpRestart::pack_darray(int n)
{
